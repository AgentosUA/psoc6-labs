...
/** The define for SROM to indicate that ANA_CTL0 VCC SEL should be set */
#define SYSPM_SET_ANA_CTL0_VCC_SEL      (0x30000101U)

/** The define for SROM to indicate that ANA_CTL0 VCC SEL should be cleared */
#define SYSPM_CLEAR_ANA_CTL0_VCC_SEL    (0x30000001U)
        ...
        */
    ...
    #endif /* defined(CY_DEVICE_PSOC6ABLE2) && !defined(CY_PSOC6ABLE2_REV_0A_SUPPORT_DISABLE) */
        {
            ...
            */
            ...
            {
                SRSS->PWR_TRIM_WAKE_CTL = SFLASH->PWR_TRIM_WAKE_CTL;
            }
            else
            {
    ...
    cy_en_syspm_status_t Cy_SysPm_BuckEnable(cy_en_syspm_buck_voltage1_t voltage)
    {
        ...
        */
        if (Cy_SysPm_LdoIsEnabled())
        {
            ...
            /* The Buck voltage the voltage can be 
            *  changed only if the protection context is set to zero (PC = 0) or 
            *  the device revision supports modifying registers via syscall.
            */
            if (IsVoltageChangePossible())
            {
                ...
                if (CY_SYSPM_BUCK_OUT1_VOLTAGE_0_9V == voltage)
                {
                    ...
                    */
                    ...
                    if (CY_SYSPM_SUCCESS == retVal)
                    {
                        ...
                        /* Set LDO output voltage to 0.95 V nominal */
                        SRSS->PWR_TRIM_PWRSYS_CTL = _CLR_SET_FLD32U((SRSS->PWR_TRIM_PWRSYS_CTL), 
                ...
                {
                    /* Read current LDO voltage */
                    cy_en_syspm_ldo_voltage_t curLdoVoltage = Cy_SysPm_LdoGetVoltage();

                    if (CY_SYSPM_LDO_VOLTAGE_1_1V != curLdoVoltage)
                    {
                        /* Set LDO to 0.95 V */
                        SRSS->PWR_TRIM_PWRSYS_CTL = _CLR_SET_FLD32U((SRSS->PWR_TRIM_PWRSYS_CTL), 
                        SRSS_PWR_TRIM_PWRSYS_CTL_ACT_REG_TRIM, LDO_OUT_VOLTAGE_0_95V);
                        ...
                        /* Set LDO to 1.1 V */
                        SRSS->PWR_TRIM_PWRSYS_CTL = _CLR_SET_FLD32U((SRSS->PWR_TRIM_PWRSYS_CTL), 
                        SRSS_PWR_TRIM_PWRSYS_CTL_ACT_REG_TRIM, LDO_OUT_VOLTAGE_1_1V);
...
* 
* \param value
* Value to be set in the VCC_SEL bit for the FLASHC_FM_CTL_ANA_CTL0 register. 
...
static cy_en_syspm_status_t WriteVoltageBitForFlash(uint32_t value)
{
    ...
    *  update if protection context is = 0 */
    if (Cy_Prot_GetActivePC(ACTIVE_BUS_MASTER) == 0U)
    {    
        #if (defined (CY_IP_M4CPUSS_VERSION) && (CY_IP_M4CPUSS_VERSION >= 2u))
        #else
            FLASHC_FM_CTL->ANA_CTL0 = _CLR_SET_FLD32U((FLASHC_FM_CTL->ANA_CTL0), FLASHC_FM_CTL_ANA_CTL0_VCC_SEL, value);
        #endif
    ...
    else
    {
        ...
        uint32_t syscallCode = (0U != value) ? SYSPM_SET_ANA_CTL0_VCC_SEL : SYSPM_CLEAR_ANA_CTL0_VCC_SEL;
...
* set to zero (PC = 0), or the device revision supports modifying registers via
* syscall
*
*******************************************************************************/
static bool IsVoltageChangePossible(void)
{
    ...
    bool hasHandleRegSyscall = (uint32_t) Cy_SysLib_GetDeviceRevision() > SYSPM_DEVICE_REV_0B;

    return (hasHandleRegSyscall || (curProtContext == 0U));
}
